package tk.kanaostore.losteddev.skywars.menu.shop.visual_cosmetics.trail;
...
public class ProjectileTrailsMenu extends PagedPlayerMenu {
    ...
    private static final ConfigMenu config = ConfigMenu.getByName("projectiletrail");
        ...
        if (evt.getInventory().equals(getCurrentInventory())) {
            ...
            if (evt.getWhoClicked() instanceof Player && evt.getWhoClicked().equals(player)) {
                ...
                if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(evt.getInventory()) && item != null && item.getType() != Material.AIR) {
                    SkyWarsTrail trail = trails.get(item);
                    if (evt.getSlot() == this.previousPage) {
                        ...
                        if (evt.getClick().name().contains("RIGHT")) {
                            ...
                            new ProjectileTrailsMenu(player, next, filter, searchQuery);
                        } else if (evt.getClick().name().contains("LEFT")) {
                            Filter next = Filter.next(filter);
                            new ProjectileTrailsMenu(player, order, next, searchQuery);
                            ...
                            sessions.prompt(Arrays.asList(new String[] {"&aType the value you want: ", "&aType 'cancel' to cancel!"}), (input) -> {
                                ...
                                Bukkit.getScheduler().runTask(Main.getInstance(),
                                        () -> {
                                            new ProjectileTrailsMenu(player, order, filter, input);
                                        });
                            });
                        ...
                        if (!trail.has(account)) {
                            ...
                            if (!trail.canBeSold()) {
                                player.sendMessage(StringUtils.formatColors(config.getAsString("unavailable").replace("{name}", trail.getRawName())));
                                return;
                            }
    ...
    private Map<ItemStack, SkyWarsTrail> trails;
    ...
    public ProjectileTrailsMenu(Player player, Order order, Filter filter, String searchQuery) {
        ...
        List<Cosmetic> cosmetics = CosmeticServer.SKYWARS.getByType(CosmeticType.SKYWARS_TRAIL);
        ...
        for (Cosmetic c : cosmetics) {
            SkyWarsTrail trail = (SkyWarsTrail) c;
