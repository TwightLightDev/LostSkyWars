package tk.kanaostore.losteddev.skywars.menu.shop.visual_cosmetics.killmessage;
...
public class KillMessageMenu extends PagedPlayerMenu {
                ...
                if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(evt.getInventory()) && item != null && item.getType() != Material.AIR) {
                    ...
                    if (evt.getSlot() == this.previousPage) {
                        ...
                        if (evt.getClick().name().contains("RIGHT")) {
                            ...
                            new KillMessageMenu(player, next, filter, searchQuery);
                        } else if (evt.getClick().name().contains("LEFT")) {
                            Filter next = Filter.next(filter);
                            new KillMessageMenu(player, order, next, searchQuery);
                            ...
                            sessions.prompt(Arrays.asList(new String[] {"&aType the value you want: ", "&aType 'cancel' to cancel!"}), (input) -> {
                                if (input.equals("cancel")) {
                                    ...
                                    Bukkit.getScheduler().runTask(Main.getInstance(),
                                            () -> {
                                                new KillMessageMenu(player, order, filter, searchQuery);
                                            });
                                    return;
                                }
                                sessions.end();
                                Bukkit.getScheduler().runTask(Main.getInstance(),
                                        () -> {
                                            new KillMessageMenu(player, order, filter, input);
                                        });
                            });
    ...
    public KillMessageMenu(Player player, Order order, Filter filter, String searchQuery) {
        ...
        List<Cosmetic> cosmetics = CosmeticServer.SKYWARS.getByType(CosmeticType.SKYWARS_KILLMESSAGE);
