...
public class SQLiteDatabase extends Database {
    ...
    public void migrateFromLegacy() {
        ...
        LOGGER.info("Legacy tables detected (SQLite). Starting migration...");
        ...
        try {
            ...
            CachedRowSet swRs = tableExists("premium_lostedskywars") ? this.query("SELECT * FROM premium_lostedskywars") : null;
            ...
            for (Map.Entry<String, String> entry : uuidToName.entrySet()) {
                ...
                this.update("INSERT OR IGNORE INTO profile (uuid, name, coins, souls, level, exp, max_souls, well_roll, souls_per_win, mystery_dusts, deliveries, leveling, elo, brave_points) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                ...
                if (sw != null) {
                    this.update("INSERT OR IGNORE INTO stats (uuid, group_id, kills, wins, assists, deaths, plays, melee_kills, bow_kills, mob_kills, void_kills) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            uuid, "solo",
                    ...
                    this.update("INSERT OR IGNORE INTO cosmetics (uuid, kits, perks, cages, death_cries, trails, balloons, kill_messages, kill_effects, sprays, victory_dances, titles, symbols) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            uuid,
                    ...
                    this.update("INSERT OR IGNORE INTO selections (uuid, kit, perk, cage, death_cry, trail, balloon, kill_message, kill_effect, spray, victory_dance, title, symbol, last_selected, favorites) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            uuid,
    ...
    private boolean tableExists(String tableName) {
        CachedRowSet rs = query("SELECT name FROM sqlite_master WHERE type='table' AND name=?", tableName);
    ...
    private void renameLegacyTables() {
        if (tableExists("premium_lostedaccount")) {
            this.update("ALTER TABLE premium_lostedaccount RENAME TO premium_lostedaccount_backup");
        }
        if (tableExists("premium_lostedskywars")) {
            this.update("ALTER TABLE premium_lostedskywars RENAME TO premium_lostedskywars_backup");
        }
        if (tableExists("ranked_lostedskywars")) {
            this.update("ALTER TABLE ranked_lostedskywars RENAME TO ranked_lostedskywars_backup");
        }
    ...
    public Map<String, ValueContainer> loadProfile(UUID uuid, String name) {
        ...
        this.execute("INSERT INTO `profile` (uuid, name, coins, souls, level, exp, max_souls, well_roll, souls_per_win, mystery_dusts, deliveries, leveling, elo, brave_points) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                uuid.toString(), name, 0, 0, 1, 0.0, 100, 1, 0, 0, "{}", "[]", 0, 0);
        return defaults;
    }
    ...
    public CompletableFuture<Account> loadAccountOffline(String name) {
        return CompletableFuture.supplyAsync(() -> {
            CachedRowSet rs = query("SELECT * FROM `profile` WHERE LOWER(`name`) = ?", name.toLowerCase());
    ...
    public void openConnection() {
        try {
            ...
            boolean reconnect = connection != null;
            connection = DriverManager.getConnection("jdbc:sqlite:" + file);
            LOGGER.info(reconnect ? "Reconnected on SQLite!" : "Connected to SQLite!");
    ...
    public boolean isConnected() {
        try {
            ...
            LOGGER.log(Level.SEVERE, "Could not check SQLite connection: ", ex);
            return false;
        }
    }
