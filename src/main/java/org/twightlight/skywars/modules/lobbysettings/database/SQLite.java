package org.twightlight.skywars.modules.lobbysettings.database;
...
public class SQLite {
    ...
    public void connect() {
        ...
        try {
            ...
            statement.executeUpdate(" CREATE TABLE IF NOT EXISTS "
                    ...
                    "fly TEXT DEFAULT 'false', " +
                    "speed INTEGER DEFAULT 0, " +
                    "jumpboost INTEGER DEFAULT 0, " +
    ...
    public Connection getConnection() {
        File modules = new File(plugin.getDataFolder().getPath() + "/modules/lobbysettings");
        if (!modules.exists()) {
            modules.mkdirs();
        }
        File dataFile = new File(plugin.getDataFolder().getPath() + "/modules/lobbysettings", dbname + ".db");
        if (!dataFile.exists())
            try {
    ...
    public void createPlayerData(OfflinePlayer p) {
        try (Connection connection = getConnection();
             ...
             PreparedStatement insertPs = connection.prepareStatement("INSERT INTO " + dbname +
                     " (player, fly, speed, jumpboost, vanish, showParticles, showScoreboard) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
            ...
            if (!rs.next()) {
                ...
                insertPs.setString(2, "false");
                insertPs.setInt(3, 0);
                insertPs.setInt(4, 0);
                insertPs.setString(5, "false");
