...
public class AcceptCmd extends SubCommand {
    public AcceptCmd(Friends module, Permission permission) {
    ...
    public void execute(CommandSender user, String... args) {
        if (!validatePermissions(user))
            return;
        if (args.length == 1) {
            getModule().getMessageUtil().message(user, getModule().getConfig().getString("messages.invalid-arguments").replace("%0%", "friend accept <name>"));
            return;
        }
        ...
        request.accept(getModule());
        for (String line : getModule().getConfig().getStringList("messages.request-accepted"))
            getModule().getMessageUtil().message(user, line.replace("%0%", potentialFriend.isOnline() ? Bukkit.getPlayer(args[1]).getDisplayName() : potentialFriend.getName()));
    }
}
