...
public class SQLite {
    ...
    public Connection getConnection() {
        File modules = new File(plugin.getDataFolder().getPath() + "/modules/boosters");
        if (!modules.exists()) {
            modules.mkdirs();
        }
        File dataFile = new File(plugin.getDataFolder().getPath() + "/modules/boosters", dbname + ".db");
        if (!dataFile.exists())
            try {
    ...
    public boolean addColumn(String dbname, String columnName, String columnType, String defaultValue) {

        try (Connection conn = getConnection();
             Statement stmt = conn.createStatement()) {

            if (!columnExists(dbname, columnName)) {
    ...
    private boolean columnExists(String dbname, String columnName) throws SQLException {
