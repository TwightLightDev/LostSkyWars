...
public class SoulWellMenu extends PlayerMenu {
                    ...
                    if (action != null && !action.getType().equals("NOTHING")) {
                        if (action.getType().equals("OPEN")) {
                            ...
                            if (menu.equalsIgnoreCase("roll")) {
                                if (account.getSouls() < (10 * account.getWellRoll())) {
                                    player.sendMessage(StringUtils.formatColors(config.getAsString("dont-have-enough-souls")));
                                    return;
                                }

                                new RollSoulWellMenu(player, back, account.getWellRoll());
    ...
    public SoulWellMenu(Player player, boolean back) {
        ...
        Account account = Database.getInstance().getAccount(player.getUniqueId());
        int cost = account.getWellRoll() * 10;
        this.back = back;
        ...
        for (Map.Entry<Integer, ConfigItem> entry : config.getItems().entrySet()) {
            if (entry.getKey() >= 0 && entry.getKey() < this.getInventory().getSize()) {
                ...
                // ROLL
                stack = stack.replace("{color}", cost > account.getSouls() ? "&c" : "&a");
                stack = stack.replace("{cost}", StringUtils.formatNumber(cost));
                stack = stack.replace("{message}", cost > account.getSouls() ? config.getAsString("dont-have-enough-souls") : config.getAsString("have-enough-souls"));
