package org.twightlight.skywars.menu.shop.ingamecosmetics.spray;
...
public class SpraysMenu extends PagedPlayerMenu {
        ...
        if (evt.getInventory().equals(getCurrentInventory())) {
            ...
            if (evt.getWhoClicked() instanceof Player && evt.getWhoClicked().equals(player)) {
                ...
                if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(evt.getInventory()) && item != null && item.getType() != Material.AIR) {
                    SkyWarsSpray cos = sprs.get(item);
                    if (evt.getSlot() == this.previousPage) {
                        ...
                        if (evt.getClick().name().contains("RIGHT")) {
                            ...
                            new SpraysMenu(player, next, filter, searchQuery);
                        } else if (evt.getClick().name().contains("LEFT")) {
                            Filter next = Filter.next(filter);
                            new SpraysMenu(player, order, next, searchQuery);
    ...
    private Map<ItemStack, SkyWarsSpray> sprs;
    ...
    public SpraysMenu(Player player) {
        this(player, Order.NONE, Filter.ALL, "");
    }

    public SpraysMenu(Player player, Order order, Filter filter, String searchQuery) {
        ...
        List<VisualCosmetic> cosmetics = CosmeticServer.SKYWARS.getByType(CosmeticType.SKYWARS_SPRAY);
        ...
        for (VisualCosmetic c : cosmetics) {
            SkyWarsSpray trail = (SkyWarsSpray) c;
