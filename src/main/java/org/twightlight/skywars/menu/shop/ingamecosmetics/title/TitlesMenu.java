package org.twightlight.skywars.menu.shop.ingamecosmetics.title;
...
import org.twightlight.skywars.cosmetics.visual.categories.SkyWarsTitle;
...
public class TitlesMenu extends PagedPlayerMenu {
                ...
                if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(evt.getInventory()) && item != null && item.getType() != Material.AIR) {
                    ...
                    if (evt.getSlot() == this.previousPage) {
                        ...
                        if (evt.getClick().name().contains("RIGHT")) {
                            ...
                            new TitlesMenu(player, next, filter, searchQuery);
                        } else if (evt.getClick().name().contains("LEFT")) {
                            Filter next = Filter.next(filter);
                            new TitlesMenu(player, order, next, searchQuery);
                            ...
                            sessions.prompt(Arrays.asList(new String[] {"&aType the value you want: ", "&aType 'cancel' to cancel!"}), (input) -> {
                                if (input.equals("cancel")) {
                                    ...
                                    Bukkit.getScheduler().runTask(SkyWars.getInstance(),
                                            () -> {
                                                new TitlesMenu(player, order, filter, searchQuery);
                                            });
                                    return;
                                }
                                sessions.end();
                                Bukkit.getScheduler().runTask(SkyWars.getInstance(),
                                        () -> {
                                            new TitlesMenu(player, order, filter, input);
                                        });
                            });
                        ...
                        if (!cos.has(account)) {
                            ...
                            new ConfirmMenu(player, cos, TitlesMenu.class, order, filter, searchQuery);
                            return;
                        }
    ...
    private Map<ItemStack, SkyWarsTitle> ts;
    ...
    public TitlesMenu(Player player) {
        this(player, Order.NONE, Filter.ALL, "");
    }

    public TitlesMenu(Player player, Order order, Filter filter, String searchQuery) {
        ...
        List<VisualCosmetic> cosmetics = CosmeticServer.SKYWARS.getByType(CosmeticType.SKYWARS_TITLE);
        ...
        for (VisualCosmetic c : cosmetics) {
            SkyWarsTitle cos = (SkyWarsTitle) c;
