package org.twightlight.skywars.menu.shop.ingamecosmetics.victorydance;
...
public class VictoryDancesMenu extends PagedPlayerMenu {
        ...
        if (evt.getInventory().equals(getCurrentInventory())) {
            ...
            if (evt.getWhoClicked() instanceof Player && evt.getWhoClicked().equals(player)) {
                ...
                if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(evt.getInventory()) && item != null && item.getType() != Material.AIR) {
                    SkyWarsVictoryDance cos = vds.get(item);
                    if (evt.getSlot() == this.previousPage) {
                        ...
                        if (evt.getClick().name().contains("RIGHT")) {
                            ...
                            new VictoryDancesMenu(player, next, filter, searchQuery);
                        } else if (evt.getClick().name().contains("LEFT")) {
                            Filter next = Filter.next(filter);
                            new VictoryDancesMenu(player, order, next, searchQuery);
    ...
    private Map<ItemStack, SkyWarsVictoryDance> vds;
    ...
    public VictoryDancesMenu(Player player) {
        this(player, Order.NONE, Filter.ALL, "");
    }

    public VictoryDancesMenu(Player player, Order order, Filter filter, String searchQuery) {
        super(player, config.getTitle(), config.getRows());
        this.vds = new HashMap<>();
        ...
        List<VisualCosmetic> cosmetics = CosmeticServer.SKYWARS.getByType(CosmeticType.SKYWARS_VICTORYDANCE);
        ...
        for (VisualCosmetic c : cosmetics) {
            SkyWarsVictoryDance cos = (SkyWarsVictoryDance) c;
