package org.twightlight.skywars.menu.shop.ingamecosmetics.cage;
...
public class CagesMenu extends PagedPlayerMenu {
            ...
            if (evt.getWhoClicked() instanceof Player && evt.getWhoClicked().equals(player)) {
                ...
                if (evt.getClickedInventory() != null && evt.getClickedInventory().equals(evt.getInventory()) && item != null && item.getType() != Material.AIR) {
                    ...
                    if (evt.getSlot() == this.previousPage) {
                        ...
                        if (cage.selected(account)) {
                            player.sendMessage(StringUtils.formatColors(config.getAsString("deselect").replace("{name}", cage.getRawName())));
                            account.getSelectedContainer().setGlobalSelection(VisualCosmeticType.CAGE.getSelectionColumn(), 0);
                            new CagesMenu(player, order, filter, searchQuery);
                            return;
                        }

                        player.sendMessage(StringUtils.formatColors(config.getAsString("select").replace("{name}", cage.getRawName())));
                        account.getSelectedContainer().setGlobalSelection(VisualCosmeticType.CAGE.getSelectionColumn(), cage.getId());
                        new CagesMenu(player, order, filter, searchQuery);
                    } else {
    ...
    public CagesMenu(Player player, Order order, Filter filter, String searchQuery) {
        ...
        List<VisualCosmetic> cosmetics = VisualCosmetic.listByType(VisualCosmeticType.CAGE);
