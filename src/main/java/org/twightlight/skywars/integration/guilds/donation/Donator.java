package org.twightlight.skywars.integration.guilds.donation;
...
import org.twightlight.skywars.integration.guilds.level.Level;
...
public class Donator {
    ...
    public Donator(Player p) {
        ...
        next_refresh = Long.parseLong(GuildsIntegration.getExternalDB().
                getData(p, "nextRefresh", new TypeToken<String>() {}, "0"));
        if (System.currentTimeMillis() > next_refresh) {
            GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), 0, "donation_today");
        }
        while (System.currentTimeMillis() > next_refresh) {
            next_refresh += 86_400_000;
        }
        GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), next_refresh, "nextRefresh");

        donation_today = GuildsIntegration.getExternalDB().getData(p, "donation_today", new TypeToken<Integer>() {}, 0);
        donation_limit = GuildsIntegration.getExternalDB().getData(p, "donation_limit", new TypeToken<Integer>() {}, 1000);
        ratio = GuildsIntegration.getExternalDB().getData(p, "ratio", new TypeToken<Double>() {}, 0.75D);
        coins = GuildsIntegration.getExternalDB().getData(p, "coins", new TypeToken<Integer>() {}, 0);
    ...
    public void donate(int donation_amount) {
        if (account != null) {
            ...
            int finalAmount;
            if (left <= 0) {
                GuildsIntegration.getLanguage().getList("guilds.donation.limit-reached").forEach(line -> account.getPlayer().sendMessage(ChatColor.translateAlternateColorCodes('&', line)));
                return;
            }
            ...
            GuildsIntegration.getLanguage().getList("guilds.donation.success").
            ...
            GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), donation_today, "donation_today");
            GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), coins, "coins");
        }
    }
    ...
    public void saveData() {
        GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), String.valueOf(next_refresh), "nextRefresh");
        GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), donation_limit, "donation_limit");
        GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), donation_today, "donation_today");
        GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), ratio, "ratio");
        GuildsIntegration.getExternalDB().updateData(Bukkit.getPlayer(uuid), coins, "coins");
        level.saveData();
    }
