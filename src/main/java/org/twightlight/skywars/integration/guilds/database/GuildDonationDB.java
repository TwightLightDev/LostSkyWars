package org.twightlight.skywars.integration.guilds.database;
...
public class GuildDonationDB {
    private Connection connection;
    private Plugin plugin;
    private String dbname;

    public GuildDonationDB(Plugin plugin, String name) {
        this.plugin = plugin;
        this.dbname = name;
        connect();
    }
        ...
        try {
            ...
            statement.executeUpdate(
                    "CREATE TABLE IF NOT EXISTS " + dbname + " (" +
                            ...
                            "nextRefresh TEXT DEFAULT '0', " +
                            "donation_limit INTEGER DEFAULT 1000, " +
                            "donation_today INTEGER DEFAULT 0, " +
                            "ratio REAL DEFAULT 0.75, " +
                            "currentXP INTEGER DEFAULT 0.0, " +
    ...
    public Connection getConnection() {
        File modules = new File(plugin.getDataFolder().getPath() + "/modules/guilds_expansion");
        if (!modules.exists()) {
            modules.mkdirs();
        }

        File dataFile = new File(plugin.getDataFolder().getPath() + "/modules/guilds_expansion", dbname + ".db");
        if (!dataFile.exists())
            try {
    ...
    public void createPlayerData(OfflinePlayer p) {
        ...
        try (Connection connection = getConnection();
             PreparedStatement checkPs = connection.prepareStatement(
                     "SELECT 1 FROM " + dbname + " WHERE player = ?");
             PreparedStatement insertPs = connection.prepareStatement(
                     "INSERT INTO " + dbname + " (player, nextRefresh, donation_limit, donation_today, ratio, currentXP, level, coins) " +
                             "VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
            ...
            if (!rs.next()) {
                ...
                insertPs.setString(2, String.valueOf(getNextMidnightMillis()));
                insertPs.setInt(3, 1000);
                insertPs.setInt(4, 0);
                insertPs.setDouble(5, 0.75);
                insertPs.setDouble(6, 0);
                insertPs.setInt(7, 1);
                insertPs.setInt(8, 0);
    ...
    public long getNextMidnightMillis() {
        ZoneId zone = TimeZone.getDefault().toZoneId();
        LocalDate tomorrow = LocalDate.now(zone).plusDays(1);
        ZonedDateTime nextMidnight = tomorrow.atStartOfDay(zone);
